...
AutoDiscovery.prototype.dockerEventHandler = function (dockerEvent, containerInfo) {
  // console.error(JSON.stringify(containerInfo, null, '  '))
  ...
  appConfig.spmToken = this.getConfigValue('SPM_TOKEN', containerInfo) || this.getConfigValue('MONITORING_TOKEN', containerInfo) || appConfig.spmToken
  appConfig.infraToken = this.getConfigValue('INFRA_TOKEN', containerInfo) || appConfig.infraToken
  ...
  if (appConfig.containerNetwork === 'host') {
    ...
    if (containerInfo.NetworkSettings.Ports &&
      ...
      console.log('containerInfo.NetworkSettings.Ports[httpsPort][0].HostIp:' + host)
    }
    if (containerInfo.NetworkSettings.Ports &&
      ...
      console.log('containerInfo.NetworkSettings.Ports[httpPort][0].HostIp:' + host)
    }
  } else {
    ...
    if (containerInfo.NetworkSettings.IPAddress && containerInfo.NetworkSettings.IPAddress.length > 6) {
      ...
      SpmAgent.Logger.debug('containerInfo.NetworkSettings.IPAddress:' + host)
    }
    // iterate over networks, assuming network[0] is the default
    var networkIndex = Object.keys(containerInfo.NetworkSettings.Networks).length - 1
    if (containerInfo.NetworkSettings && containerInfo.NetworkSettings.Networks && Object.keys(containerInfo.NetworkSettings.Networks).length > 0) {
      SpmAgent.Logger.debug('non-bridge network')
      var networks = Object.keys(containerInfo.NetworkSettings.Networks)
      if (networks && networks.length > 0 && containerInfo.NetworkSettings.Networks[networks[networkIndex]].IPAMConfig &&
        containerInfo.NetworkSettings.Networks[networks[networkIndex]].IPAMConfig.IPv4Address) {
        host = containerInfo.NetworkSettings.Networks[networks[networkIndex]].IPAMConfig.IPv4Address
        networkID = containerInfo.NetworkSettings.Networks[networks[networkIndex]].NetworkID
        SpmAgent.Logger.debug('containerInfo.NetworkSettings.Networks[networks[0]].IPAMConfig.IPv4Address:' + host)
      } else if (networks && networks.length > 0 && containerInfo.NetworkSettings.Networks[networks[networkIndex]] &&
        networks && networks.length > 0 && containerInfo.NetworkSettings.Networks[networks[networkIndex]].IPAddress) {
        host = containerInfo.NetworkSettings.Networks[networks[networkIndex]].IPAddress
        networkID = containerInfo.NetworkSettings.Networks[networks[networkIndex]].NetworkID
        SpmAgent.Logger.debug('containerInfo.NetworkSettings.Networks[networks[0]].IPAddress: ' + host)
      }
      if (containerInfo.HostConfig.NetworkMode && containerInfo.HostConfig.NetworkMode.length > 0 && containerInfo.NetworkSettings.Networks[containerInfo.HostConfig.NetworkMode]) {
        ...
        SpmAgent.Logger.debug('containerInfo.NetworkSettings.Networks[containerInfo.HostConfig.NetworkMode].IPAddress: ' + host)
      }
    }
  }
  if (networkID !== null) {
  ...
  SpmAgent.Logger.debug(JSON.stringify(containerInfo.NetworkSettings))
  SpmAgent.Logger.debug('host: ' + host)
  return {
    host: host,
    port: port,
    networkID: networkID,
...
AutoDiscovery.prototype.kubernetesEventHandler = function (dockerEvent, containerInfo) {
  // overwrites config values by container Labels/Env - retruns a cloned config with modified values
  var appConfig = this.mergeConfig(containerInfo)
  var port = 0
  var host = null
  if (typeof appConfig.matchImageName === 'string') {
    appConfig.matchImageName = new RegExp(appConfig.matchImageName)
  }
  if (!appConfig.matchImageName.test(containerInfo.Config.Image)) {
    if (process.env.debug === 'true') {
      SpmAgent.Logger.info('No match for image name ' + appConfig.matchImageName + ' ' + containerInfo.Config.Image)
    }
    return null
  }
  if (typeof appConfig.matchContainerName === 'string') {
    appConfig.matchContainerName = new RegExp(appConfig.matchContainerName)
  }
  if (!appConfig.matchContainerName.test(containerInfo.Name)) {
    return null
  }
  appConfig.infraToken = this.getConfigValue('INFRA_TOKEN', containerInfo) || appConfig.infraToken
  appConfig.spmToken = this.getConfigValue('SPM_TOKEN', containerInfo) || this.getConfigValue('MONITORING_TOKEN', containerInfo) || appConfig.spmToken
  if (appConfig.spmToken) {
    SpmAgent.Logger.info('SPM_TOKEN set for image/container ' + containerInfo.Config.Image + containerInfo.Name)
    // SpmAgent.Logger.info('Start monioring for image/container ' + containerInfo.Config.Image + containerInfo.Name)
  } else {
    if (process.env.debug === 'true') {
      SpmAgent.Logger.info('No SPM_TOKEN set for ' + containerInfo.Config.Image + containerInfo.Name)
    }
    return null
  }
  if (appConfig.portInContainer) {
  ...
  SpmAgent.Logger.debug('Container ID: ' + dockerEvent.id, 'PodIp:' + host)
  if (!pod) {
    SpmAgent.Logger.error('no pod found for container ID: ' + dockerEvent.id)
    return null
  } else {
    this.setupAgent({
      host: host,
...
AutoDiscovery.prototype.getConfigValue = function (key, containerInfo, prefix = '') {
  ...
  if (containerInfo && containerInfo.Config && containerInfo.Config.Labels && containerInfo.Config.Environment) {
    return containerInfo.Config.Labels[prefix + key] || containerInfo.Config.Environment[prefix + key] || this.appConfig[key]
  } else {
...
AutoDiscovery.prototype.mergeConfig = function (containerInfo) {
  ...
  appConfigProperties.forEach(function (property) {
    appConfig[property] = this.getConfigValue(property, containerInfo, 'sematext_' + this.appName + '_')
  }.bind(this))
  return appConfig
}
...
AutoDiscovery.prototype.setupAgent = function (socketInfo, dockerEvent, containerInfo) {
  if (!socketInfo) {
    SpmAgent.Logger.error('no network info to start monitor')
    return
  }
  var infraToken = socketInfo.appConfig.infraToken || containerInfo.Config.Environment['INFRA_TOKEN'] || containerInfo.Config.Labels['infra-token'] || process.env.INFRA_TOKEN || process.env['infra-token']
  /**
  if (infraToken) {
    infraToken = '--infraToken ' + infraToken
  } else {
    infraToken = ''
  }
  **/
  if (socketInfo.port > 0) {
    ...
    var templateContext = {
      ...
      spmToken: socketInfo.appConfig.spmToken || containerInfo.Config.Labels['monitoring-token'] || containerInfo.Config.Environment['MONITORING_TOKEN'],
      region: containerInfo.Config.Environment['REGION'] || containerInfo.Config.Labels['region'] || process.env.REGION || 'US',
    ...
    if (!templateContext.spmToken) {
      SpmAgent.Logger.info('Missing SPM token for ' + containerInfo.Image + '/' + containerInfo.Name)
      return
    }
    ...
    if (socketInfo.appConfig.spmConfig) {
      Object.keys(socketInfo.appConfig.spmConfig).forEach(function (key) {
        if (socketInfo.appConfig.spmConfig[key] !== null) {
          templateContext.spmConfig = templateContext.spmConfig + ' --' + key + ' ' + format(socketInfo.appConfig.spmConfig[key], templateContext)
        }
      })
    }
    ...
    SpmAgent.Logger.info(cmd)
    try {
      SpmAgent.Logger.debug(exec(cmd).toString())
      ...
      SpmAgent.Logger.debug(dockerEvent.id + ' ' + JSON.stringify(runningAgents[dockerEvent.id]))
      // no spm-client service restart required for Node.js based agents
...
AutoDiscovery.prototype.start = function (dockerEvent, containerInfo) {
  ...
  startEventCache.set(dockerEvent.id, {
    containerInfo: containerInfo,
    dockerEvent: dockerEvent,
    containerId: dockerEvent.id
  })
  if (globalConfig.debug.printEvents) {
    SpmAgent.Logger.info(JSON.stringify(dockerEvent, null, ' '))
  }
  if (globalConfig.debug.printContainerInfo) {
    SpmAgent.Logger.info(JSON.stringify(containerInfo))
  }
...
function startAutoDiscovery () {
  var appWatchers = []
  Object.keys(rcConfig.templates).forEach(function (key) {
    var autodisco = new AutoDiscovery(rcConfig, rcConfig.templates[key], key)
    appWatchers.push(autodisco)
  })

  var stream = null
  function restartPodMonitor () {
    SpmAgent.Logger.log('lost connection to k8s, restart pod monitor')
  ...
  if (process.env.KUBERNETES_PORT_443_TCP || process.env.LOCAL_k8S_TEST) {
    useKubernetesApi = true
    SpmAgent.Logger.log('Start watching Kuberntes pod status events')
    try {
      stream = new PodStatusMonitor(containerIps, podIps)
      if (stream) {
        ...
        stream.on('data', function (data) {
          ...
          appWatchers.forEach(function (aw) {
            try {
              aw.kubernetesEventHandler(dockerInfo.dockerEvent, dockerInfo.containerInfo)
            } catch (err) {
              SpmAgent.Logger.error('Error in kubernetesEventHandler:' + err)
            }
          })
        })
      ...
      SpmAgent.Logger.error(err.toString())
    }
  }
