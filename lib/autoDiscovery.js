  ...
  var dockerDiscovery = new DockerDiscovery({
    ...
    startFunction: function (dockerEvent, containerInfo) {
      ...
      if (appConfig.dockerNetwork === 'host') {
        ...
        if (containerInfo.NetworkSettings.IPAddress && containerInfo.NetworkSettings.IPAddress.length > 0) {
          host = containerInfo.NetworkSettings.IPAddress
        } else {
          // a different network might be used, specified in HostConfig.NetworkMode
          if (containerInfo.HostConfig.NetworkMode && containerInfo.HostConfig.NetworkMode.length > 0) {
            SpmAgent.Logger.info('Using container network ' + containerInfo.HostConfig.NetworkMode)
