...
var monitorPodStatus = require('./podStatusMonitor')
...
AutoDiscovery.prototype.start = function (dockerEvent, containerInfo) {
  ...
  if (appConfig.containerNetwork === 'host') {
    ...
    if (containerInfo.NetworkSettings.IPAddress && containerInfo.NetworkSettings.IPAddress.length > 6) {
      ...
      if (containerInfo.NetworkSettings.Networks && containerInfo.NetworkSettings.Networks.length > 0) {
  ...
  if (port > 0) {
    ...
    var templateContext = {
      ...
      host: host,
      port: port,
      podName: pod ? pod.podFullName : null,
      process: process
    }
...
function restartPodMonitor () {
  stream = monitorPodStatus(containerIps, podIps)
  if (stream) {
...
// delay the start of docker event collection until running podIP's are collected
setTimeout(function () {
